...
export function ShelterPage() {
  ...
  useEffect(() => {
    axios.get("http://localhost:5000/api/shelters")
      .then(res => {
        ...
        const past = res.data.filter(s => s.Status !== "Active");
        setActiveShelters(active);
        setPastShelters(past);
      })
  ...
  const handleSaveShelter = (updatedData, action) => {
    if (action === "delete") {
      setActiveShelters(prev => prev.filter(s => s.Shelter_id !== selectedShelter.Shelter_id));
      setPastShelters(prev => prev.filter(s => s.Shelter_id !== selectedShelter.Shelter_id));
    } else {
      setActiveShelters(prev => prev.map(s => 
        s.Shelter_id === selectedShelter.Shelter_id ? { ...s, ...updatedData } : s
      ));
      setPastShelters(prev => prev.map(s => 
        s.Shelter_id === selectedShelter.Shelter_id ? { ...s, ...updatedData } : s
      ));
    }
  ...
  return (
    <div className="shelters-app">
      <aside className="sidebar">
        <div className="brand"><div className="brand-icon">আশ্রয়</div></div>
        <nav className="side-nav">
          ...
          <NavLink to="/events" className={({ isActive }) => isActive ? "nav-item active" : "nav-item"}>
            <i className="fa-solid fa-bell" /><span>Events</span>
          </NavLink>
          <NavLink to="/shelters" className={({ isActive }) => isActive ? "nav-item active" : "nav-item"}>
            <i className="fa-solid fa-house-chimney" /><span>Shelters</span>
          </NavLink>
          <a className="nav-item" href="#"><i className="fa-solid fa-users" /><span>Volunteers</span></a>
          <a className="nav-item" href="#"><i className="fa-solid fa-dollar-sign" /><span>Donations</span></a>
          <a className="nav-item" href="#"><i className="fa-solid fa-boxes-stacked" /><span>Inventory</span></a>
        </nav>
      </aside>
      ...
      <div className="shelters-main">
        ...
        <div className="shelters-body">
          <section className="section">
            ...
            <div className="cards-grid">
              {activeShelters.map(s => (
                <article key={s.Shelter_id} className="event-card" onClick={() => { setSelectedShelter(s); setShowViewCardModal(true); }}>
                  <div className="card-img"><img src={shelterImg} alt={s.Shelter_name}/></div>
                  <div className="card-info">
                    <div className="card-title">{s.Shelter_name}</div>
                    <div className="card-area">{s.Area_name}</div>
                    <div className="card-meta">
                      <div><span className="meta-label">Total Capacity: </span>{s.Total_capacity}</div>
                      <div><span className="meta-label">Current Capacity: </span>{s.Current_capacity}</div>
                    </div>
                  </div>
                </article>
              ))}
          ...
          {showPopup && <div className="popup-backdrop"><AddShelter header="Shelter" handleState={() => setShowPopup(false)} /></div>}
          {showFilterModal && <div className="popup-backdrop"><FilterModal handleState={() => setShowFilterModal(false)} /></div>}
